...
* `max_value` Validate that the number provided is no greater than this value. It must be `Decimal` type.
* `min_value` Validate that the number provided is no less than this value. It must be `Decimal` type.
